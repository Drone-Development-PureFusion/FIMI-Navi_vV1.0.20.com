.class Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;
.super Lcom/twitter/sdk/android/core/identity/AuthHandler;
.source "SSOAuthHandler.java"


# static fields
.field static final DOGFOOD_PACKAGE_NAME:Ljava/lang/String; = "com.twitter.android.beta"

.field static final DOGFOOD_SIGNATURE:Ljava/lang/String; = ""

.field private static final EXTRA_CONSUMER_KEY:Ljava/lang/String; = "ck"

.field private static final EXTRA_CONSUMER_SECRET:Ljava/lang/String; = "cs"

.field static final SSO_CLASS_NAME:Ljava/lang/String; = "com.twitter.android.SingleSignOnActivity"

.field static final TWITTER_PACKAGE_NAME:Ljava/lang/String; = "com.twitter.android"

.field static final TWITTER_SIGNATURE:Ljava/lang/String; = ""


# direct methods
.method public constructor <init>(Lcom/twitter/sdk/android/core/TwitterAuthConfig;Lcom/twitter/sdk/android/core/Callback;I)V
    .locals 0
    .param p1, "authConfig"    # Lcom/twitter/sdk/android/core/TwitterAuthConfig;
    .param p3, "requestCode"    # I
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lcom/twitter/sdk/android/core/TwitterAuthConfig;",
            "Lcom/twitter/sdk/android/core/Callback",
            "<",
            "Lcom/twitter/sdk/android/core/TwitterSession;",
            ">;I)V"
        }
    .end annotation

    .prologue
    .line 95
    .local p2, "callback":Lcom/twitter/sdk/android/core/Callback;, "Lcom/twitter/sdk/android/core/Callback<Lcom/twitter/sdk/android/core/TwitterSession;>;"
    invoke-direct {p0, p1, p2, p3}, Lcom/twitter/sdk/android/core/identity/AuthHandler;-><init>(Lcom/twitter/sdk/android/core/TwitterAuthConfig;Lcom/twitter/sdk/android/core/Callback;I)V

    .line 96
    return-void
.end method

.method public static availableSSOPackage(Landroid/content/pm/PackageManager;)Ljava/lang/String;
    .locals 2
    .param p0, "pm"    # Landroid/content/pm/PackageManager;

    .prologue
    .line 137
    const-string v0, "com.twitter.android"

    const-string v1, ""

    invoke-static {p0, v0, v1}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->checkAppSignature(Landroid/content/pm/PackageManager;Ljava/lang/String;Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_0

    .line 138
    const-string v0, "com.twitter.android"

    .line 142
    :goto_0
    return-object v0

    .line 139
    :cond_0
    const-string v0, "com.twitter.android.beta"

    const-string v1, ""

    invoke-static {p0, v0, v1}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->checkAppSignature(Landroid/content/pm/PackageManager;Ljava/lang/String;Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_1

    .line 140
    const-string v0, "com.twitter.android.beta"

    goto :goto_0

    .line 142
    :cond_1
    const/4 v0, 0x0

    goto :goto_0
.end method

.method private static checkAppSignature(Landroid/content/pm/PackageManager;Ljava/lang/String;Ljava/lang/String;)Z
    .locals 8
    .param p0, "pm"    # Landroid/content/pm/PackageManager;
    .param p1, "packageName"    # Ljava/lang/String;
    .param p2, "requiredSignature"    # Ljava/lang/String;

    .prologue
    const/4 v3, 0x0

    .line 161
    const/16 v4, 0x40

    :try_start_0
    invoke-virtual {p0, p1, v4}, Landroid/content/pm/PackageManager;->getPackageInfo(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;
    :try_end_0
    .catch Landroid/content/pm/PackageManager$NameNotFoundException; {:try_start_0 .. :try_end_0} :catch_0

    move-result-object v1

    .line 165
    .local v1, "p":Landroid/content/pm/PackageInfo;
    iget-object v5, v1, Landroid/content/pm/PackageInfo;->signatures:[Landroid/content/pm/Signature;

    array-length v6, v5

    move v4, v3

    :goto_0
    if-ge v4, v6, :cond_1

    aget-object v2, v5, v4

    .line 166
    .local v2, "s":Landroid/content/pm/Signature;
    invoke-virtual {v2}, Landroid/content/pm/Signature;->toCharsString()Ljava/lang/String;

    move-result-object v7

    invoke-virtual {p2, v7}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v7

    if-nez v7, :cond_0

    .line 170
    .end local v1    # "p":Landroid/content/pm/PackageInfo;
    .end local v2    # "s":Landroid/content/pm/Signature;
    :goto_1
    return v3

    .line 162
    :catch_0
    move-exception v0

    .line 163
    .local v0, "e":Landroid/content/pm/PackageManager$NameNotFoundException;
    goto :goto_1

    .line 165
    .end local v0    # "e":Landroid/content/pm/PackageManager$NameNotFoundException;
    .restart local v1    # "p":Landroid/content/pm/PackageInfo;
    .restart local v2    # "s":Landroid/content/pm/Signature;
    :cond_0
    add-int/lit8 v4, v4, 0x1

    goto :goto_0

    .line 170
    .end local v2    # "s":Landroid/content/pm/Signature;
    :cond_1
    const/4 v3, 0x1

    goto :goto_1
.end method

.method public static isAvailable(Landroid/content/Context;)Z
    .locals 3
    .param p0, "context"    # Landroid/content/Context;

    .prologue
    .line 152
    invoke-virtual {p0}, Landroid/content/Context;->getPackageManager()Landroid/content/pm/PackageManager;

    move-result-object v0

    .line 153
    .local v0, "pm":Landroid/content/pm/PackageManager;
    const-string v1, "com.twitter.android"

    const-string v2, ""

    invoke-static {v0, v1, v2}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->checkAppSignature(Landroid/content/pm/PackageManager;Ljava/lang/String;Ljava/lang/String;)Z

    move-result v1

    if-nez v1, :cond_0

    const-string v1, "com.twitter.android.beta"

    const-string v2, ""

    .line 154
    invoke-static {v0, v1, v2}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->checkAppSignature(Landroid/content/pm/PackageManager;Ljava/lang/String;Ljava/lang/String;)Z

    move-result v1

    if-eqz v1, :cond_1

    :cond_0
    const/4 v1, 0x1

    .line 153
    :goto_0
    return v1

    .line 154
    :cond_1
    const/4 v1, 0x0

    goto :goto_0
.end method

.method private startAuthActivityForResult(Landroid/app/Activity;)Z
    .locals 11
    .param p1, "activity"    # Landroid/app/Activity;

    .prologue
    const/4 v10, 0x0

    const/4 v6, 0x0

    .line 105
    invoke-virtual {p1}, Landroid/app/Activity;->getPackageManager()Landroid/content/pm/PackageManager;

    move-result-object v4

    .line 106
    .local v4, "pm":Landroid/content/pm/PackageManager;
    invoke-static {v4}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->availableSSOPackage(Landroid/content/pm/PackageManager;)Ljava/lang/String;

    move-result-object v3

    .line 107
    .local v3, "packageName":Ljava/lang/String;
    if-nez v3, :cond_0

    .line 108
    invoke-static {}, Lcom/twitter/sdk/android/core/Twitter;->getLogger()Lcom/twitter/sdk/android/core/Logger;

    move-result-object v7

    const-string v8, "Twitter"

    const-string v9, "SSO app signature check failed"

    invoke-interface {v7, v8, v9, v10}, Lcom/twitter/sdk/android/core/Logger;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    .line 127
    :goto_0
    return v6

    .line 112
    :cond_0
    new-instance v5, Landroid/content/ComponentName;

    const-string v7, "com.twitter.android.SingleSignOnActivity"

    invoke-direct {v5, v3, v7}, Landroid/content/ComponentName;-><init>(Ljava/lang/String;Ljava/lang/String;)V

    .line 113
    .local v5, "ssoActivity":Landroid/content/ComponentName;
    invoke-virtual {p0}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->getAuthConfig()Lcom/twitter/sdk/android/core/TwitterAuthConfig;

    move-result-object v0

    .line 114
    .local v0, "authConfig":Lcom/twitter/sdk/android/core/TwitterAuthConfig;
    new-instance v7, Landroid/content/Intent;

    invoke-direct {v7}, Landroid/content/Intent;-><init>()V

    invoke-virtual {v7, v5}, Landroid/content/Intent;->setComponent(Landroid/content/ComponentName;)Landroid/content/Intent;

    move-result-object v2

    .line 115
    .local v2, "intent":Landroid/content/Intent;
    invoke-static {p1, v2}, Lcom/twitter/sdk/android/core/IntentUtils;->isActivityAvailable(Landroid/content/Context;Landroid/content/Intent;)Z

    move-result v7

    if-nez v7, :cond_1

    .line 116
    invoke-static {}, Lcom/twitter/sdk/android/core/Twitter;->getLogger()Lcom/twitter/sdk/android/core/Logger;

    move-result-object v7

    const-string v8, "Twitter"

    const-string v9, "SSO auth activity not found"

    invoke-interface {v7, v8, v9, v10}, Lcom/twitter/sdk/android/core/Logger;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    goto :goto_0

    .line 119
    :cond_1
    const-string v7, "ck"

    invoke-virtual {v0}, Lcom/twitter/sdk/android/core/TwitterAuthConfig;->getConsumerKey()Ljava/lang/String;

    move-result-object v8

    invoke-virtual {v2, v7, v8}, Landroid/content/Intent;->putExtra(Ljava/lang/String;Ljava/lang/String;)Landroid/content/Intent;

    move-result-object v7

    const-string v8, "cs"

    .line 120
    invoke-virtual {v0}, Lcom/twitter/sdk/android/core/TwitterAuthConfig;->getConsumerSecret()Ljava/lang/String;

    move-result-object v9

    invoke-virtual {v7, v8, v9}, Landroid/content/Intent;->putExtra(Ljava/lang/String;Ljava/lang/String;)Landroid/content/Intent;

    .line 123
    :try_start_0
    iget v7, p0, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->requestCode:I

    invoke-virtual {p1, v2, v7}, Landroid/app/Activity;->startActivityForResult(Landroid/content/Intent;I)V
    :try_end_0
    .catch Ljava/lang/Exception; {:try_start_0 .. :try_end_0} :catch_0

    .line 124
    const/4 v6, 0x1

    goto :goto_0

    .line 125
    :catch_0
    move-exception v1

    .line 126
    .local v1, "e":Ljava/lang/Exception;
    invoke-static {}, Lcom/twitter/sdk/android/core/Twitter;->getLogger()Lcom/twitter/sdk/android/core/Logger;

    move-result-object v7

    const-string v8, "Twitter"

    const-string v9, "SSO exception occurred"

    invoke-interface {v7, v8, v9, v1}, Lcom/twitter/sdk/android/core/Logger;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    goto :goto_0
.end method


# virtual methods
.method public authorize(Landroid/app/Activity;)Z
    .locals 1
    .param p1, "activity"    # Landroid/app/Activity;

    .prologue
    .line 100
    invoke-direct {p0, p1}, Lcom/twitter/sdk/android/core/identity/SSOAuthHandler;->startAuthActivityForResult(Landroid/app/Activity;)Z

    move-result v0

    return v0
.end method
