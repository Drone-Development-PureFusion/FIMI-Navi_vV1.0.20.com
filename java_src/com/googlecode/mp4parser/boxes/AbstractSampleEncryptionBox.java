...
public abstract class AbstractSampleEncryptionBox extends AbstractFullBox {
    ...
    private static void ajc$preClinit() {
        ...
        ajc$tjp_3 = factory.makeSJP(JoinPoint.METHOD_EXECUTION, factory.makeMethodSig("1", "equals", "com.googlecode.mp4parser.boxes.AbstractSampleEncryptionBox", "java.lang.Object", "o", "", "boolean"), 173);
        ...
        ajc$tjp_5 = factory.makeSJP(JoinPoint.METHOD_EXECUTION, factory.makeMethodSig("1", "getEntrySizes", "com.googlecode.mp4parser.boxes.AbstractSampleEncryptionBox", "", "", "", "java.util.List"), JfifUtil.MARKER_RST0);
    }
        ...
        while (true) {
            ...
            if (remainingNumOfEntries > 0) {
                try {
                    ...
                    e.f290iv = new byte[ivSize];
                    content.get(e.f290iv);
                    if ((getFlags() & 2) > 0) {
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        for (CencSampleAuxiliaryDataFormat entry : this.entries) {
            if (entry.getSize() > 0) {
                if (entry.f290iv.length != 8 && entry.f290iv.length != 16) {
                    throw new RuntimeException("IV must be either 8 or 16 bytes");
                }
                byteBuffer.put(entry.f290iv);
    ...
    public List<Short> getEntrySizes() {
        ...
        for (CencSampleAuxiliaryDataFormat entry : this.entries) {
            short size = (short) entry.f290iv.length;
            if (isSubSampleEncryption()) {
