...
public class TextTrackImpl extends AbstractTrack {
    ...
    public synchronized List<Sample> getSamples() {
        if (this.samples == null) {
            ...
            for (Line sub : this.subs) {
                ...
                try {
                    dos.writeShort(sub.text.getBytes(SocketOption.DEFAULT_CHARSET).length);
                    dos.write(sub.text.getBytes(SocketOption.DEFAULT_CHARSET));
                    dos.close();
    ...
    public long[] getSampleDurations() {
        ...
        for (Line sub : this.subs) {
            ...
            decTimes.add(Long.valueOf(sub.f266to - sub.from));
            lastEnd = sub.f266to;
        }
