    ...
    public SilenceTrackImpl(Track ofType, long ms) {
        ...
        if (AudioSampleEntry.TYPE3.equals(ofType.getSampleDescriptionBox().getSampleEntry().getType())) {
            ...
            while (true) {
                ...
                if (numFrames2 > 0) {
                    this.samples.add(new SampleImpl((ByteBuffer) ByteBuffer.wrap(new byte[]{33, 16, 4, FcCollection.MSG_SET_FOLLOW_OPEN, -116, FcCollection.MSG_SET_FC_RETURE_HOME_RESUME}).rewind()));
                } else {
                    return;
                }
            }
