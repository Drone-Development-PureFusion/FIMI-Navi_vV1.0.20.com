...
public final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    public void run() {
        if (!waitIfPaused() && !delayIfNeed()) {
            ...
            C2073L.m1299d(LOG_START_DISPLAY_IMAGE_TASK, this.memoryCacheKey);
            if (loadFromUriLock.isLocked()) {
                C2073L.m1299d(LOG_WAITING_FOR_IMAGE_LOADED, this.memoryCacheKey);
            }
                ...
                if (bmp == null || bmp.isRecycled()) {
                    ...
                    if (bmp != null) {
                        ...
                        if (this.options.shouldPreProcess()) {
                            C2073L.m1299d(LOG_PREPROCESS_IMAGE, this.memoryCacheKey);
                            bmp = this.options.getPreProcessor().process(bmp);
                            if (bmp == null) {
                                C2073L.m1298e(ERROR_PRE_PROCESSOR_NULL, this.memoryCacheKey);
                            }
                        }
                        if (bmp != null && this.options.isCacheInMemory()) {
                            C2073L.m1299d(LOG_CACHE_IMAGE_IN_MEMORY, this.memoryCacheKey);
                    ...
                    C2073L.m1299d(LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING, this.memoryCacheKey);
                }
                if (bmp != null && this.options.shouldPostProcess()) {
                    C2073L.m1299d(LOG_POSTPROCESS_IMAGE, this.memoryCacheKey);
                    bmp = this.options.getPostProcessor().process(bmp);
                    if (bmp == null) {
                        C2073L.m1298e(ERROR_POST_PROCESSOR_NULL, this.memoryCacheKey);
                    }
                }
                checkTaskNotActual();
                checkTaskInterrupted();
    ...
    private boolean waitIfPaused() {
        ...
        if (pause.get()) {
            synchronized (this.engine.getPauseLock()) {
                if (pause.get()) {
                    C2073L.m1299d(LOG_WAITING_FOR_RESUME, this.memoryCacheKey);
                    try {
                        this.engine.getPauseLock().wait();
                        C2073L.m1299d(LOG_RESUME_AFTER_PAUSE, this.memoryCacheKey);
                    } catch (InterruptedException e) {
                        C2073L.m1298e(LOG_TASK_INTERRUPTED, this.memoryCacheKey);
                        return true;
                    }
                }
            }
    ...
    private boolean delayIfNeed() {
        if (this.options.shouldDelayBeforeLoading()) {
            C2073L.m1299d(LOG_DELAY_BEFORE_LOADING, Integer.valueOf(this.options.getDelayBeforeLoading()), this.memoryCacheKey);
            try {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (imageFile2 != null && imageFile2.exists() && imageFile2.length() > 0) {
                C2073L.m1299d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, this.memoryCacheKey);
                this.loadedFrom = LoadedFrom.DISC_CACHE;
                checkTaskNotActual();
                bitmap = decodeImage(ImageDownloader.Scheme.FILE.wrap(imageFile2.getAbsolutePath()));
            }
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                C2073L.m1299d(LOG_LOAD_IMAGE_FROM_NETWORK, this.memoryCacheKey);
    ...
    private boolean tryCacheImageOnDisk() throws TaskCancelledException {
        C2073L.m1299d(LOG_CACHE_IMAGE_ON_DISK, this.memoryCacheKey);
        try {
            ...
            if (loaded) {
                ...
                if (width > 0 || height > 0) {
                    C2073L.m1299d(LOG_RESIZE_CACHED_IMAGE_FILE, this.memoryCacheKey);
    ...
    private boolean downloadImage() throws IOException {
        ...
        if (is == null) {
            C2073L.m1298e(ERROR_NO_IMAGE_STREAM, this.memoryCacheKey);
        } else {
            try {
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        if (bmp != null && this.configuration.processorForDiskCache != null) {
            C2073L.m1299d(LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK, this.memoryCacheKey);
            bmp = this.configuration.processorForDiskCache.process(bmp);
            if (bmp == null) {
                C2073L.m1298e(ERROR_PROCESSOR_FOR_DISK_CACHE_NULL, this.memoryCacheKey);
            }
        }
        if (bmp == null) {
            return false;
    ...
    private boolean fireProgressEvent(final int current, final int total) {
        ...
        if (this.progressListener != null) {
            Runnable r = new Runnable() { // from class: com.nostra13.universalimageloader.core.LoadAndDisplayImageTask.1
                @Override // java.lang.Runnable
                public void run() {
                    LoadAndDisplayImageTask.this.progressListener.onProgressUpdate(LoadAndDisplayImageTask.this.uri, LoadAndDisplayImageTask.this.imageAware.mo1856getWrappedView(), current, total);
                }
            };
        ...
        if (!this.syncLoading && !isTaskInterrupted() && !isTaskNotActual()) {
            Runnable r = new Runnable() { // from class: com.nostra13.universalimageloader.core.LoadAndDisplayImageTask.2
                ...
                public void run() {
                    ...
                    LoadAndDisplayImageTask.this.listener.onLoadingFailed(LoadAndDisplayImageTask.this.uri, LoadAndDisplayImageTask.this.imageAware.mo1856getWrappedView(), new FailReason(failType, failCause));
                }
            };
            runTask(r, false, this.handler, this.engine);
        }
    ...
    private void fireCancelEvent() {
        if (!this.syncLoading && !isTaskInterrupted()) {
            Runnable r = new Runnable() { // from class: com.nostra13.universalimageloader.core.LoadAndDisplayImageTask.3
                @Override // java.lang.Runnable
                public void run() {
                    LoadAndDisplayImageTask.this.listener.onLoadingCancelled(LoadAndDisplayImageTask.this.uri, LoadAndDisplayImageTask.this.imageAware.mo1856getWrappedView());
                }
            };
            runTask(r, false, this.handler, this.engine);
        }
    ...
    private boolean isTaskInterrupted() {
        if (Thread.interrupted()) {
            C2073L.m1299d(LOG_TASK_INTERRUPTED, this.memoryCacheKey);
            return true;
        }
        return false;
    }
