...
public class BaseImageDecoder implements ImageDecoder {
    ...
    public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
        ...
        if (imageStream == null) {
            C2073L.m1298e(ERROR_NO_IMAGE_STREAM, decodingInfo.getImageKey());
            return null;
        }
        try {
            ...
            if (decodedBitmap == null) {
                C2073L.m1298e(ERROR_CANT_DECODE_IMAGE, decodingInfo.getImageKey());
    ...
    protected ExifInfo defineExifOrientation(String imageUri) {
        ...
        try {
            ...
            int exifOrientation = exif.getAttributeInt(android.support.media.ExifInterface.TAG_ORIENTATION, 1);
            ...
            C2073L.m1294w("Can't read EXIF tags from file [%s]", imageUri);
        }
    ...
    protected BitmapFactory.Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
        ...
        if (scale > 1 && this.loggingEnabled) {
            C2073L.m1299d(LOG_SUBSAMPLE_IMAGE, imageSize, imageSize.scaleDown(scale), Integer.valueOf(scale), decodingInfo.getImageKey());
        }
        ...
        if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
            ...
            if (Float.compare(scale, 1.0f) != 0) {
                ...
                if (this.loggingEnabled) {
                    C2073L.m1299d(LOG_SCALE_IMAGE, srcSize, srcSize.scale(scale), Float.valueOf(scale), decodingInfo.getImageKey());
                }
            }
        }
        if (flipHorizontal) {
            ...
            if (this.loggingEnabled) {
                C2073L.m1299d(LOG_FLIP_IMAGE, decodingInfo.getImageKey());
            }
        }
        if (rotation != 0) {
            m.postRotate(rotation);
            if (this.loggingEnabled) {
                C2073L.m1299d(LOG_ROTATE_IMAGE, Integer.valueOf(rotation), decodingInfo.getImageKey());
            }
        }
