...
final class MarkableInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final InputStream f298in;
    private long limit;
    private long offset;
    private long reset;
    ...
    public MarkableInputStream(InputStream in, int size) {
        this.defaultMark = -1L;
        this.f298in = !in.markSupported() ? new BufferedInputStream(in, size) : in;
    }
    ...
    private void setLimit(long limit) {
        try {
            if (this.reset < this.offset && this.offset <= this.limit) {
                this.f298in.reset();
                this.f298in.mark((int) (limit - this.reset));
                skip(this.reset, this.offset);
            } else {
                this.reset = this.offset;
                this.f298in.mark((int) (limit - this.offset));
            }
            this.limit = limit;
        } catch (IOException e) {
    ...
    private void skip(long current, long target) throws IOException {
        while (current < target) {
            long skipped = this.f298in.skip(target - current);
            if (skipped == 0) {
                if (read() != -1) {
                    skipped = 1;
                } else {
    ...
    public int read(byte[] buffer) throws IOException {
        int count = this.f298in.read(buffer);
        if (count != -1) {
            this.offset += count;
        }
        return count;
    ...
    public int read(byte[] buffer, int offset, int length) throws IOException {
        int count = this.f298in.read(buffer, offset, length);
        if (count != -1) {
            this.offset += count;
        }
        return count;
    ...
    public long skip(long byteCount) throws IOException {
        long skipped = this.f298in.skip(byteCount);
        this.offset += skipped;
        return skipped;
    }
    ...
    public boolean markSupported() {
        return this.f298in.markSupported();
    }
}
