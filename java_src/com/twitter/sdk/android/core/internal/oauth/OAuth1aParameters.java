...
class OAuth1aParameters {
    ...
    String calculateSignature(String signatureBase) {
        try {
            ...
            SecretKey secretKey = new SecretKeySpec(keyBytes, HostConstants.OAUTH_MAC_ALGORITHM);
            Mac mac = Mac.getInstance(HostConstants.OAUTH_MAC_ALGORITHM);
            mac.init(secretKey);
            byte[] signatureBytes = mac.doFinal(signatureBaseBytes);
            return ByteString.m31of(signatureBytes, 0, signatureBytes.length).base64();
        } catch (UnsupportedEncodingException e) {
            Twitter.getLogger().mo1290e("Twitter", "Failed to calculate signature", e);
            return "";
        } catch (InvalidKeyException e2) {
            Twitter.getLogger().mo1290e("Twitter", "Failed to calculate signature", e2);
            return "";
        } catch (NoSuchAlgorithmException e3) {
            Twitter.getLogger().mo1290e("Twitter", "Failed to calculate signature", e3);
            return "";
        }
    }
