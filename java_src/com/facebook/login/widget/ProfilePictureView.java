...
public class ProfilePictureView extends FrameLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (View.MeasureSpec.getMode(heightMeasureSpec) != 1073741824 && params.height == -2) {
            ...
            heightMeasureSpec = View.MeasureSpec.makeMeasureSpec(newHeight, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE);
            customMeasure = true;
        }
        if (View.MeasureSpec.getMode(widthMeasureSpec) != 1073741824 && params.width == -2) {
            newWidth = getPresetSizeInPixels(true);
            widthMeasureSpec = View.MeasureSpec.makeMeasureSpec(newWidth, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE);
            customMeasure = true;
        }
        if (customMeasure) {
    ...
    private void parseAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, C0552R.styleable.com_facebook_profile_picture_view);
        setPresetSize(a.getInt(C0552R.styleable.com_facebook_profile_picture_view_com_facebook_preset_size, -1));
        this.isCropped = a.getBoolean(C0552R.styleable.com_facebook_profile_picture_view_com_facebook_is_cropped, true);
        a.recycle();
    }
    ...
    private void setBlankProfilePicture() {
        ...
        if (this.customizedDefaultProfilePicture == null) {
            int blankImageResource = isCropped() ? C0552R.C0554drawable.com_facebook_profile_picture_blank_square : C0552R.C0554drawable.com_facebook_profile_picture_blank_portrait;
    ...
    private int getPresetSizeInPixels(boolean forcePreset) {
        ...
        switch (this.presetSizeType) {
            case -4:
                dimensionId = C0552R.dimen.com_facebook_profilepictureview_preset_size_large;
                break;
            case -3:
                dimensionId = C0552R.dimen.com_facebook_profilepictureview_preset_size_normal;
                break;
            case -2:
                dimensionId = C0552R.dimen.com_facebook_profilepictureview_preset_size_small;
                break;
            case -1:
                if (!forcePreset) {
                    return 0;
