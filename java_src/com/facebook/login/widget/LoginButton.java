...
public class LoginButton extends FacebookButtonBase {
    ...
    private void checkToolTipSettings() {
        switch (this.toolTipMode) {
            case AUTOMATIC:
                ...
                return;
            case DISPLAY_ALWAYS:
                String toolTipString = getResources().getString(C0552R.string.com_facebook_tooltip_default);
                displayToolTip(toolTipString);
                return;
            default:
                return;
    ...
    public void configureButton(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C0552R.C0553color.com_facebook_blue));
    ...
    protected int getDefaultStyleResource() {
        return C0552R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C0552R.styleable.com_facebook_login_view, defStyleAttr, defStyleRes);
        try {
            this.confirmLogout = a.getBoolean(C0552R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = a.getString(C0552R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = a.getString(C0552R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(a.getInt(C0552R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (text == null) {
            text = resources.getString(C0552R.string.com_facebook_loginview_log_in_button_long);
            int logInWidth = measureButtonWidth(text);
            int width = resolveSize(logInWidth, widthMeasureSpec);
            if (width < logInWidth) {
                text = resources.getString(C0552R.string.com_facebook_loginview_log_in_button);
            }
        }
        int logInWidth2 = measureButtonWidth(text);
        String text2 = this.logoutText;
        if (text2 == null) {
            text2 = resources.getString(C0552R.string.com_facebook_loginview_log_out_button);
        }
    ...
    public void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            setText(this.logoutText != null ? this.logoutText : resources.getString(C0552R.string.com_facebook_loginview_log_out_button));
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            String text = resources.getString(C0552R.string.com_facebook_loginview_log_in_button_long);
            int width = getWidth();
            if (width != 0) {
        ...
        public void onClick(View v) {
            ...
            if (accessToken != null) {
                if (LoginButton.this.confirmLogout) {
                    String logout = LoginButton.this.getResources().getString(C0552R.string.com_facebook_loginview_log_out_action);
                    String cancel = LoginButton.this.getResources().getString(C0552R.string.com_facebook_loginview_cancel_action);
                    Profile profile = Profile.getCurrentProfile();
                    if (profile != null && profile.getName() != null) {
                        message = String.format(LoginButton.this.getResources().getString(C0552R.string.com_facebook_loginview_logged_in_as), profile.getName());
                    } else {
                        message = LoginButton.this.getResources().getString(C0552R.string.com_facebook_loginview_logged_in_using_facebook);
                    }
