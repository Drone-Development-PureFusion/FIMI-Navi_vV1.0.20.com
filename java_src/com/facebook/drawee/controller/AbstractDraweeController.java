...
public abstract class AbstractDraweeController<T, INFO> implements DraweeController, DeferredReleaser.Releasable, GestureDetector.ClickListener {
    ...
    private void init(String id, Object callerContext, boolean justConstructed) {
        ...
        if (FLog.isLoggable(2)) {
            FLog.m1368v(TAG, "controller %x %s -> %s: initialize", Integer.valueOf(System.identityHashCode(this)), this.mId, id);
        }
        this.mId = id;
    ...
    public void setHierarchy(@Nullable DraweeHierarchy hierarchy) {
        if (FLog.isLoggable(2)) {
            FLog.m1368v(TAG, "controller %x %s: setHierarchy: %s", Integer.valueOf(System.identityHashCode(this)), this.mId, hierarchy);
        }
    ...
    public void onAttach() {
        if (FLog.isLoggable(2)) {
            FLog.m1368v(TAG, "controller %x %s: onAttach: %s", Integer.valueOf(System.identityHashCode(this)), this.mId, this.mIsRequestSubmitted ? "request already submitted" : "request needs submit");
        }
    ...
    public void onDetach() {
        if (FLog.isLoggable(2)) {
            FLog.m1369v(TAG, "controller %x %s: onDetach", Integer.valueOf(System.identityHashCode(this)), this.mId);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (FLog.isLoggable(2)) {
            FLog.m1368v(TAG, "controller %x %s: onTouchEvent %s", Integer.valueOf(System.identityHashCode(this)), this.mId, event);
        }
        if (this.mGestureDetector == null) {
            return false;
        }
    ...
    public boolean onClick() {
        if (FLog.isLoggable(2)) {
            FLog.m1369v(TAG, "controller %x %s: onClick", Integer.valueOf(System.identityHashCode(this)), this.mId);
        }
        if (shouldRetryOnTap()) {
    ...
    protected void submitRequest() {
        T closeableImage = mo1655getCachedImage();
        ...
        if (FLog.isLoggable(2)) {
            FLog.m1368v(TAG, "controller %x %s: submitRequest: dataSource: %x", Integer.valueOf(System.identityHashCode(this)), this.mId, Integer.valueOf(System.identityHashCode(this.mDataSource)));
        }
        ...
        DataSubscriber<T> dataSubscriber = new BaseDataSubscriber<T>() { // from class: com.facebook.drawee.controller.AbstractDraweeController.1
            ...
            public void onNewResultImpl(DataSource<T> dataSource) {
                ...
                T image = dataSource.mo1679getResult();
                if (image != null) {
    ...
    private void logMessageAndImage(String messageAndMethod, T image) {
        if (FLog.isLoggable(2)) {
            FLog.m1365v(TAG, "controller %x %s: %s: image: %s %x", Integer.valueOf(System.identityHashCode(this)), this.mId, messageAndMethod, getImageClass(image), Integer.valueOf(getImageHash(image)));
        }
    }

    private void logMessageAndFailure(String messageAndMethod, Throwable throwable) {
        if (FLog.isLoggable(2)) {
            FLog.m1367v(TAG, "controller %x %s: %s: failure: %s", Integer.valueOf(System.identityHashCode(this)), this.mId, messageAndMethod, throwable);
        }
    }
    ...
    public String toString() {
        return Objects.toStringHelper(this).add("isAttached", this.mIsAttached).add("isRequestSubmitted", this.mIsRequestSubmitted).add("hasFetchFailed", this.mHasFetchFailed).add("fetchedImage", getImageHash(this.mFetchedImage)).add(C2216b.f431Y, this.mEventTracker.toString()).toString();
    }

    /* renamed from: getCachedImage */
    protected T mo1655getCachedImage() {
        return null;
    }
}
