...
public class SimpleDraweeView extends GenericDraweeView {
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        if (!isInEditMode()) {
            ...
            this.mSimpleDraweeControllerBuilder = sDraweeControllerBuilderSupplier.mo1796get();
            if (attrs != null) {
                TypedArray gdhAttrs = context.obtainStyledAttributes(attrs, C0592R.styleable.SimpleDraweeView);
                try {
                    if (gdhAttrs.hasValue(C0592R.styleable.SimpleDraweeView_actualImageUri)) {
                        setImageURI(Uri.parse(gdhAttrs.getString(C0592R.styleable.SimpleDraweeView_actualImageUri)), (Object) null);
                    }
                } finally {
                    gdhAttrs.recycle();
                }
    ...
    public void setImageURI(Uri uri, @Nullable Object callerContext) {
        DraweeController controller = this.mSimpleDraweeControllerBuilder.mo1662setCallerContext(callerContext).mo1658setUri(uri).mo1663setOldController(getController()).mo1661build();
