...
public class ConstrainedExecutorService extends AbstractExecutorService {
    ...
    public void execute(Runnable runnable) {
        ...
        if (queueSize > maxSize && this.mMaxQueueSize.compareAndSet(maxSize, queueSize)) {
            FLog.m1369v(TAG, "%s: max pending work in queue = %d", this.mName, Integer.valueOf(queueSize));
        }
        startWorkerIfNeeded();
    }
    ...
    public void startWorkerIfNeeded() {
        ...
        while (currentCount < this.mMaxConcurrency) {
            ...
            if (this.mPendingWorkers.compareAndSet(currentCount, updatedCount)) {
                FLog.m1368v(TAG, "%s: starting worker %d of %d", this.mName, Integer.valueOf(updatedCount), Integer.valueOf(this.mMaxConcurrency));
                this.mExecutor.execute(this.mTaskRunner);
                return;
            }
            FLog.m1370v(TAG, "%s: race in startWorkerIfNeeded; retrying", this.mName);
    ...
    public class Worker implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (runnable == null) {
                    FLog.m1370v(ConstrainedExecutorService.TAG, "%s: Worker has nothing to run", ConstrainedExecutorService.this.mName);
                } else {
                    runnable.run();
                }
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m1369v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(workers));
                }
            } catch (Throwable th) {
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m1369v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(workers2));
                }
                throw th;
            }
        }
