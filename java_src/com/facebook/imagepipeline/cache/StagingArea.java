...
public class StagingArea {
                ...
                synchronized (storedEncodedImage) {
                    try {
                        if (!EncodedImage.isValid(storedEncodedImage)) {
                            this.mMap.remove(key);
                            FLog.m1353w(TAG, "Found closed reference %d for key %s (%d)", Integer.valueOf(System.identityHashCode(storedEncodedImage)), key.getUriString(), Integer.valueOf(System.identityHashCode(key)));
                            encodedImage = null;
                        } else {
    ...
    private synchronized void logStats() {
        FLog.m1370v(TAG, "Count = %d", Integer.valueOf(this.mMap.size()));
    }
}
