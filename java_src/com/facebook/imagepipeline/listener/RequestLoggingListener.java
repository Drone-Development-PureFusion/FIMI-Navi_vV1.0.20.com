...
public class RequestLoggingListener implements RequestListener {
    ...
    public synchronized void onRequestStart(ImageRequest request, Object callerContextObject, String requestId, boolean isPrefetch) {
        if (FLog.isLoggable(2)) {
            FLog.m1359v(TAG, "time %d: onRequestSubmit: {requestId: %s, callerContext: %s, isPrefetch: %b}", Long.valueOf(getTime()), requestId, callerContextObject, Boolean.valueOf(isPrefetch));
    ...
    public synchronized void onProducerStart(String requestId, String producerName) {
        if (FLog.isLoggable(2)) {
            ...
            FLog.m1360v(TAG, "time %d: onProducerStart: {requestId: %s, producer: %s}", Long.valueOf(startTime), requestId, producerName);
        }
    }
    ...
    public synchronized void onProducerFinishWithSuccess(String requestId, String producerName, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(2)) {
            ...
            FLog.m1357v(TAG, "time %d: onProducerFinishWithSuccess: {requestId: %s, producer: %s, elapsedTime: %d ms, extraMap: %s}", Long.valueOf(currentTime), requestId, producerName, Long.valueOf(getElapsedTime(startTime, currentTime)), extraMap);
        }
    }
    ...
    public synchronized void onProducerFinishWithFailure(String requestId, String producerName, Throwable throwable, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(5)) {
            ...
            Long startTime = this.mProducerStartTimeMap.remove(mapKey);
            long currentTime = getTime();
            FLog.m1349w(TAG, "time %d: onProducerFinishWithFailure: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s, throwable: %s}", Long.valueOf(currentTime), requestId, producerName, Long.valueOf(getElapsedTime(startTime, currentTime)), extraMap, throwable.toString());
        }
    }
    ...
    public synchronized void onProducerFinishWithCancellation(String requestId, String producerName, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(2)) {
            ...
            Long startTime = this.mProducerStartTimeMap.remove(mapKey);
            long currentTime = getTime();
            FLog.m1357v(TAG, "time %d: onProducerFinishWithCancellation: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s}", Long.valueOf(currentTime), requestId, producerName, Long.valueOf(getElapsedTime(startTime, currentTime)), extraMap);
        }
    }
    ...
    public synchronized void onProducerEvent(String requestId, String producerName, String producerEventName) {
        if (FLog.isLoggable(2)) {
            ...
            long currentTime = getTime();
            FLog.m1357v(TAG, "time %d: onProducerEvent: {requestId: %s, stage: %s, eventName: %s; elapsedTime: %d ms}", Long.valueOf(getTime()), requestId, producerName, producerEventName, Long.valueOf(getElapsedTime(startTime, currentTime)));
        }
    }
    ...
    public synchronized void onRequestSuccess(ImageRequest request, String requestId, boolean isPrefetch) {
        if (FLog.isLoggable(2)) {
            ...
            long currentTime = getTime();
            FLog.m1360v(TAG, "time %d: onRequestSuccess: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(currentTime), requestId, Long.valueOf(getElapsedTime(startTime, currentTime)));
        }
    }
    ...
    public synchronized void onRequestFailure(ImageRequest request, String requestId, Throwable throwable, boolean isPrefetch) {
        if (FLog.isLoggable(5)) {
            Long startTime = this.mRequestStartTimeMap.remove(requestId);
            long currentTime = getTime();
            FLog.m1349w(TAG, "time %d: onRequestFailure: {requestId: %s, elapsedTime: %d ms, throwable: %s}", Long.valueOf(currentTime), requestId, Long.valueOf(getElapsedTime(startTime, currentTime)), throwable.toString());
        }
    }
    ...
    public synchronized void onRequestCancellation(String requestId) {
        if (FLog.isLoggable(2)) {
            Long startTime = this.mRequestStartTimeMap.remove(requestId);
            long currentTime = getTime();
            FLog.m1360v(TAG, "time %d: onRequestCancellation: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(currentTime), requestId, Long.valueOf(getElapsedTime(startTime, currentTime)));
        }
    }
