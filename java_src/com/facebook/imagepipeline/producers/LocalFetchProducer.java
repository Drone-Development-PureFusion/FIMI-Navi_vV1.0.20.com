...
public abstract class LocalFetchProducer implements Producer<EncodedImage> {
    ...
    public EncodedImage getByteBufferBackedEncodedImage(InputStream inputStream, int length) throws IOException {
        ...
        try {
            if (length <= 0) {
                ref = CloseableReference.m1337of(this.mPooledByteBufferFactory.mo1684newByteBuffer(inputStream));
            } else {
                ref = CloseableReference.m1337of(this.mPooledByteBufferFactory.mo1685newByteBuffer(inputStream, length));
            }
