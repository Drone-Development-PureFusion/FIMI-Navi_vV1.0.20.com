...
public class EncodedMemoryCacheProducer implements Producer<EncodedImage> {
        ...
        try {
            if (cachedReference != null) {
                ...
                if (listener.requiresExtraMap(requestId)) {
                    map = ImmutableMap.m1419of("cached_value_found", ServerProtocol.DIALOG_RETURN_SCOPES_TRUE);
                }
    ...
    private static class EncodedMemoryCacheConsumer extends DelegatingConsumer<EncodedImage, EncodedImage> {
        ...
        public void onNewResultImpl(EncodedImage newResult, boolean isLast) {
            ...
            CloseableReference<PooledByteBuffer> cachedResult = newResult.getByteBufferRef();
            if (cachedResult != null) {
                try {
                    CacheKey cacheKey = newResult.getEncodedCacheKey() != null ? newResult.getEncodedCacheKey() : this.mRequestedCacheKey;
                    cachedResult = this.mMemoryCache.cache(cacheKey, cachedResult);
                    if (cachedResult != null) {
                        try {
