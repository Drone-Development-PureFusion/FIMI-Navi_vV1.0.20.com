...
public class NetworkFetchProducer implements Producer<EncodedImage> {
    ...
    public void onResponse(FetchState fetchState, InputStream responseData, int responseContentLength) throws IOException {
        ...
        if (responseContentLength > 0) {
            pooledOutputStream = this.mPooledByteBufferFactory.mo1688newOutputStream(responseContentLength);
        } else {
            pooledOutputStream = this.mPooledByteBufferFactory.mo1687newOutputStream();
        }
    ...
    private void notifyConsumer(PooledByteBufferOutputStream pooledOutputStream, boolean isFinal, Consumer<EncodedImage> consumer) {
        CloseableReference<PooledByteBuffer> result = CloseableReference.m1337of(pooledOutputStream.mo1689toByteBuffer());
