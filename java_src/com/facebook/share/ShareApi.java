...
public final class ShareApi {
    ...
    private void sharePhotoContent(SharePhotoContent photoContent, final FacebookCallback<Sharer.Result> callback) {
        ...
        try {
            for (SharePhoto photo : photoContent.getPhotos()) {
                ...
                if (bitmap != null) {
                    requests.add(GraphRequest.newUploadPhotoRequest(accessToken, getGraphPath(PHOTOS_EDGE), bitmap, caption, photo.getParameters(), requestCallback));
                } else if (photoUri != null) {
                    requests.add(GraphRequest.newUploadPhotoRequest(accessToken, getGraphPath(PHOTOS_EDGE), photoUri, caption, photo.getParameters(), requestCallback));
                }
            }
    ...
    private void shareLinkContent(ShareLinkContent linkContent, final FacebookCallback<Sharer.Result> callback) {
        ...
        parameters.putString(ActionConst.REF_ATTRIBUTE, linkContent.getRef());
