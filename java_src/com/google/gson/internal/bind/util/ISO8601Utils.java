...
public class ISO8601Utils {
    ...
    public static String format(Date date, boolean millis, TimeZone tz) {
        ...
        formatted.append(CoreConstants.DASH_CHAR);
        padInt(formatted, calendar.get(2) + 1, "MM".length());
        formatted.append(CoreConstants.DASH_CHAR);
        padInt(formatted, calendar.get(5), "dd".length());
        formatted.append('T');
        padInt(formatted, calendar.get(11), "hh".length());
        formatted.append(CoreConstants.COLON_CHAR);
        padInt(formatted, calendar.get(12), "mm".length());
        formatted.append(CoreConstants.COLON_CHAR);
        padInt(formatted, calendar.get(13), "ss".length());
        if (millis) {
            formatted.append(CoreConstants.DOT);
            padInt(formatted, calendar.get(14), "sss".length());
        }
        int offset = tz.getOffset(calendar.getTimeInMillis());
        if (offset != 0) {
            int hours = Math.abs((offset / CoreConstants.MILLIS_IN_ONE_MINUTE) / 60);
            int minutes = Math.abs((offset / CoreConstants.MILLIS_IN_ONE_MINUTE) % 60);
    ...
    */
    public static Date parse(String date, ParsePosition pos) throws ParseException {
        ...
        try {
            ...
            if (checkOffset(date, offset3, CoreConstants.DASH_CHAR)) {
                offset3++;
            }
            int offset4 = offset3 + 2;
            int month = parseInt(date, offset3, offset4);
            int offset5 = checkOffset(date, offset4, CoreConstants.DASH_CHAR) ? offset4 + 1 : offset4;
            ...
            if (hasT) {
                ...
                if (checkOffset(date, offset8, CoreConstants.COLON_CHAR)) {
                    offset8++;
                }
                int offset9 = offset8 + 2;
                minutes = parseInt(date, offset8, offset9);
                int offset10 = checkOffset(date, offset9, CoreConstants.COLON_CHAR) ? offset9 + 1 : offset9;
                if (date.length() <= offset10 || (c = date.charAt(offset10)) == 'Z' || c == '+' || c == '-') {
                    ...
                    if (checkOffset(date, offset6, CoreConstants.DOT)) {
            ...
            fail = e;
            String input = date == null ? null : CoreConstants.DOUBLE_QUOTE_CHAR + date + "'";
            msg = fail.getMessage();
