...
/* renamed from: com.fimi.libperson.ui.setting.ServiceSettingActivity */
...
public class ServiceSettingActivity extends BasePersonActivity implements AdapterView.OnItemClickListener, BitmapLoadTaskInstance.OnLoadListener {
    ...
    public void initData() {
        ...
        this.mTitleView.setTvTitle(getString(C1647R.string.libperson_service_select_title));
    ...
    public void doTrans() {
        ...
        for (int i = 0; i < ServiceItem.getServicename().length; i++) {
            ...
            if (C1647R.string.region_Argentina == this.serviceID) {
                item.setCountryCode("es");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_United_Arab_Emirates == this.serviceID) {
                item.setCountryCode("ar-SA");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_The_US == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_other == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_Mexico == this.serviceID) {
                item.setCountryCode("es");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_Saudi_Arabia == this.serviceID) {
                item.setCountryCode("ar-SA");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_Iran == this.serviceID) {
                item.setCountryCode("fa-IR");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_Poland == this.serviceID) {
                item.setCountryCode("pl");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_germany == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_France == this.serviceID) {
                item.setCountryCode(ConstantLanguages.FRANCE);
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_Slovakia == this.serviceID) {
                item.setCountryCode("sk-SK");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_Turkey == this.serviceID) {
                item.setCountryCode("tr-TR");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_Ukraine == this.serviceID) {
                item.setCountryCode("uk-UA");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_Spain == this.serviceID) {
                item.setCountryCode("es");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_Philippines == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_Malaysia == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_Burma == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_Thailand == this.serviceID) {
                item.setCountryCode("th-TH");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_Singapore == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_Indonesia == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_Vietnam == this.serviceID) {
                item.setCountryCode("vi-VN");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_hk == this.serviceID) {
                item.setCountryCode("tw");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_tw == this.serviceID) {
                item.setCountryCode("cn");
                item.setServiceUrl(ServiceItem.chinaService);
            } else if (C1647R.string.region_Mainland_China == this.serviceID) {
                item.setCountryCode("cn");
                item.setServiceUrl(ServiceItem.chinaService);
            } else if (C1647R.string.region_Russia == this.serviceID) {
                item.setCountryCode("ru");
                item.setServiceUrl(ServiceItem.moscowService);
            } else if (C1647R.string.region_maroc == this.serviceID) {
                item.setCountryCode("ar-SA");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_uk == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_italy == this.serviceID) {
                item.setCountryCode("it-IT");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_israel == this.serviceID) {
                item.setCountryCode("ar-SA");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_greco == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_japan == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_portugal == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_korea == this.serviceID) {
                item.setCountryCode("");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_colombia == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.newusService);
            } else if (C1647R.string.region_pakistan == this.serviceID) {
                item.setCountryCode("en");
                item.setServiceUrl(ServiceItem.singaporeService);
            } else if (C1647R.string.region_egpyt == this.serviceID) {
                item.setCountryCode("ar-SA");
                item.setServiceUrl(ServiceItem.frankfurtService);
            } else if (C1647R.string.region_belarus == this.serviceID) {
                item.setCountryCode("ru");
                item.setServiceUrl(ServiceItem.frankfurtService);
            }
        ...
        if (this.mPosition != position || SPStoreManager.getInstance().getObject(Constants.SERVICE_ITEM_KEY, ServiceItem.class) == null) {
            ...
            if (this.isSetting) {
                ...
                if (serviceItem.getServiceUrl().equalsIgnoreCase(this.serviceItems.get(this.mPosition).getServiceUrl())) {
                    this.dialogManager = new DialogManager(this.mContext, "", getString(C1647R.string.libperson_service_dialog_exit_message), getString(C1647R.string.libperson_service_ok), getString(C1647R.string.person_setting_dialog_exit_left_text));
                } else {
                    this.dialogManager = new DialogManager(this.mContext, "", getString(C1647R.string.libperson_service_dialog_exit_switch_message), getString(C1647R.string.libperson_service_ok), getString(C1647R.string.person_setting_dialog_exit_left_text));
                }
