...
public class LoginPresenter {
    ...
    public void getVerificationCode(String phone) {
        if (!DataValidatorUtil.isMobile(phone)) {
            this.mView.getCodeResult(false, this.mContext.getString(C1647R.string.register_input_right_phone));
        } else {
            UserManager.getIntance(this.mContext).getSecurityCode(phone, "0", "0", new DisposeDataHandle(new DisposeDataListener() { // from class: com.fimi.libperson.presenter.LoginPresenter.1
                ...
                public void onFailure(Object reasonObj) {
                    LoginPresenter.this.mView.getCodeResult(false, LoginPresenter.this.mContext.getString(C1647R.string.network_exception));
                }
            }));
        }
    }
    ...
    public void loginByPhone(final String phone, String code) {
        if (TextUtils.isEmpty(phone)) {
            this.mView.iphoneLoginResult(false, this.mContext.getString(C1647R.string.login_hint_iphone));
        } else if (!DataValidatorUtil.isMobile(phone)) {
            this.mView.iphoneLoginResult(false, this.mContext.getString(C1647R.string.register_input_right_phone));
        } else {
            UserManager.getIntance(this.mContext).loginFmUser(phone, code, "0", new DisposeDataHandle(new DisposeDataListener() { // from class: com.fimi.libperson.presenter.LoginPresenter.2
                ...
                @Override // com.fimi.kernel.network.okhttp.listener.DisposeDataListener
                public void onFailure(Object reasonObj) {
                    LoginPresenter.this.mView.iphoneLoginResult(false, LoginPresenter.this.mContext.getString(C1647R.string.network_exception));
                }
            }));
        }
    }
    ...
    public void loginByEmail(final String email, String password) {
        if (TextUtils.isEmpty(email)) {
            this.mView.emailLoginResult(false, this.mContext.getString(C1647R.string.register_email_not_null));
        } else if (!DataValidatorUtil.isEmail(email)) {
            this.mView.emailLoginResult(false, this.mContext.getString(C1647R.string.register_input_right_email));
        } else {
            UserManager.getIntance(this.mContext).loginFmUser(email, password, "1", new DisposeDataHandle(new DisposeDataListener() { // from class: com.fimi.libperson.presenter.LoginPresenter.3
                ...
                @Override // com.fimi.kernel.network.okhttp.listener.DisposeDataListener
                public void onFailure(Object reasonObj) {
                    LoginPresenter.this.mView.emailLoginResult(false, LoginPresenter.this.mContext.getString(C1647R.string.network_exception));
                }
            }));
        }
    }
