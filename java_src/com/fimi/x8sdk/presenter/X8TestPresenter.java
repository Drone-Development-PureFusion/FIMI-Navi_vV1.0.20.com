...
public class X8TestPresenter extends BasePresenter {
    ...
    public void onDataCallBack(int groupId, int msgId, ILinkMessage packet) {
        ...
        if (groupId == 16) {
            ...
            if (msgId == 2) {
                CameraRequestUpdate reqUpdate = (CameraRequestUpdate) packet;
                this.updateView.reqestUpdate(reqUpdate.getMsgRpt());
            }
            if (msgId == 3) {
    ...
    public void requestUploadFile() {
        ...
        byte[] fileSize = ByteHexHelper.intToFourHexBytes(fileBytes.length);
        sendCmd(new FwUpdateCollection().requestUploadFile(fileSize, byteCrc));
    }
