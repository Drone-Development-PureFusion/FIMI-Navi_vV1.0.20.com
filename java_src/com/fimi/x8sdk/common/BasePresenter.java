    ...
    public void onNormalFormatResonse(boolean isAck, ILinkMessage packet, BaseCommand bcd) {
        if (isAck) {
            if (isNotNull(packet.getUiCallBack())) {
                packet.getUiCallBack().onComplete(new CmdResult(true, C1846R.string.cmd_success), packet);
                return;
            }
            return;
        }
        bcd.getUiCallBack().onComplete(new CmdResult(false, C1846R.string.cmd_timeout), null);
    }

    public void onNormalResponse(boolean isAck, ILinkMessage packet, BaseCommand bcd) {
        if (isAck) {
            if (isNotNull(packet.getUiCallBack())) {
                packet.getUiCallBack().onComplete(new CmdResult(true, C1846R.string.cmd_success), null);
                return;
            }
            return;
        }
        ...
        if (isAck) {
            if (isNotNull(packet.getUiCallBack())) {
                ...
                if (this.context != null && packet.getMsgRpt() != 69) {
                    String s = this.context.getString(C1846R.string.cmd_fail) + " Error Code=" + packet.getMsgRpt();
                    X8ToastUtil.showToast(this.context, s, 0);
                }
                packet.getUiCallBack().onComplete(new CmdResult(false, C1846R.string.cmd_fail, packet.getMsgRpt()), packet);
                return;
            }
            return;
        }
