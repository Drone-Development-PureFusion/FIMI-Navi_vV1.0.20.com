...
public class PlayBackMapFragment extends SupportMapFragment implements GoogleMap.InfoWindowAdapter, GoogleMapSyncReady, OnMapReadyCallback {
    ...
    public void handlerHomePoint(AutoHomeInfoPlayback autoHomeInfoPlayback) {
        ...
        if (this.homeMarker == null) {
            if (this.mMap != null) {
                this.homeMarker = this.mMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(C0885R.C0888drawable.home_point)).position(latLng));
                return;
            }
            return;
        }
    ...
    private Marker createDroneMarker(LatLng latLng) {
        ...
        return this.mMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(C0885R.C0888drawable.icon_fly_handpiece_location)).position(latLng));
    }
    ...
    public View getInfoWindow(Marker marker) {
        View view = LayoutInflater.from(getActivity()).inflate(C0885R.layout.play_back_inforwindow, (ViewGroup) null);
        TextView droneLocation = (TextView) view.findViewById(C0885R.C0889id.drone_location);
