...
public class PhotoSubParamsAdapter extends RecyclerView.Adapter {
    ...
    public RecyclerView.ViewHolder mo1873onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 1) {
            View subView = LayoutInflater.from(this.context).inflate(C0885R.layout.x8_iso_recycler_item_title, parent, false);
            RecyclerView.ViewHolder viewHolder = new CameraParamsTitleViewHolder(subView);
            return viewHolder;
        }
        View subView2 = LayoutInflater.from(this.context).inflate(C0885R.layout.x8_photo_sub_param_list_item, parent, false);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof SubParamsViewHolder) {
            ...
            if (this.subEntity.getTitleName().equals(this.context.getResources().getString(C0885R.string.x8_camera_style))) {
                String optionName = this.subEntity.getOptions().get(position);
                if (optionName.equals(this.context.getResources().getString(C0885R.string.x8_camera_saturation)) || optionName.equals(this.context.getResources().getString(C0885R.string.x8_camera_contrast))) {
                    subParamsViewHolder.initViewStub();
                }
                if (optionName.equals(this.context.getResources().getString(C0885R.string.x8_camera_sharpness))) {
                    subParamsViewHolder.initSharpViewStub();
                }
            } else if (this.subEntity.getTitleName().equals(this.context.getResources().getString(C0885R.string.x8_photo_mode)) && this.subEntity.getOptions().get(position).equals("panorama_capture")) {
            ...
            if (position == 0) {
                holder.itemView.findViewById(C0885R.C0889id.item_back_btn).setOnClickListener(new View.OnClickListener() { // from class: com.fimi.app.x8s.adapter.PhotoSubParamsAdapter.2
                    @Override // android.view.View.OnClickListener
                    public void onClick(View v) {
