...
import com.fimi.app.x8s.tensortfloow.Classifier;
    ...
    public void tfLiteApiModel() {
        ...
        if (this.isTouch) {
            ...
            if (isDianXuan == 1) {
                ...
                RectF roiRect2 = new RectF(this.rectX, this.rectY, this.rectX + this.rectW, this.rectY + this.rectH);
                if (this.isLog) {
                    TcpClient.getIntance().sendLog("选框屏幕坐标：-<" + roiRect2.toString() + "->    ");
                }
                try {
                    roiRect = new RectF(x1, y1, x2, y2);
                } catch (Exception e) {
                    e = e;
                }
                try {
                    if (this.isLog) {
                        TcpClient.getIntance().sendLog("识别图片在PFV坐标： -<" + roiRect + "->   ");
                    }
