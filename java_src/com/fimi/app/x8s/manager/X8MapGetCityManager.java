    ...
    public static void onSetHomeEvent(final X8sMainActivity activity, final int type) {
        ...
        if (activity.getmMapVideoController().getFimiMap().hasHomeInfo()) {
            ...
            if (type == 0) {
                ...
                if (latLng == null) {
                    X8ToastUtil.showToast(activity, activity.getString(C0885R.string.x8_general_return_person_failed), 0);
                    return;
                } else {
                    lat = latLng[0];
                    lng = latLng[1];
            ...
            activity.getFcManager().setHomePoint(h, lat, lng, type, accuracy, new UiCallBackListener() { // from class: com.fimi.app.x8s.manager.X8MapGetCityManager.1
                ...
                public void onComplete(CmdResult cmdResult, Object o) {
                    if (cmdResult.isSuccess()) {
                        if (type != 0) {
                            X8ToastUtil.showToast(activity, activity.getString(C0885R.string.x8_general_return_person), 0);
                        } else {
                            X8ToastUtil.showToast(activity, activity.getString(C0885R.string.x8_general_return_drone), 0);
                        }
                    } else if (o == null) {
                        X8ToastUtil.showToast(activity, activity.getString(C0885R.string.x8_general_return_failed), 0);
                    }
                }
            });
        }
