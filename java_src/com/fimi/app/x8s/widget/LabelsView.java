...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    private static final int KEY_DATA = C0885R.C0889id.tag_key_data;
    private static final int KEY_POSITION = C0885R.C0889id.tag_key_position;
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, C0885R.styleable.labels_view);
            int type = mTypedArray.getInt(C0885R.styleable.labels_view_selectType, 1);
            this.mSelectType = SelectType.get(type);
            this.mMaxSelect = mTypedArray.getInteger(C0885R.styleable.labels_view_maxSelect, 0);
            this.mTextColor = mTypedArray.getColorStateList(C0885R.styleable.labels_view_labelTextColor);
            this.mTextSize = mTypedArray.getDimension(C0885R.styleable.labels_view_labelTextSize, sp2px(context, 14.0f));
            this.mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(C0885R.styleable.labels_view_labelTextPaddingLeft, 0);
            this.mTextPaddingTop = mTypedArray.getDimensionPixelOffset(C0885R.styleable.labels_view_labelTextPaddingTop, 0);
            this.mTextPaddingRight = mTypedArray.getDimensionPixelOffset(C0885R.styleable.labels_view_labelTextPaddingRight, 0);
            this.mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(C0885R.styleable.labels_view_labelTextPaddingBottom, 0);
            this.mLineMargin = mTypedArray.getDimensionPixelOffset(C0885R.styleable.labels_view_lineMargin, 0);
            this.mWordMargin = mTypedArray.getDimensionPixelOffset(C0885R.styleable.labels_view_wordMargin, 0);
            int labelBgResId = mTypedArray.getResourceId(C0885R.styleable.labels_view_labelBackground, 0);
            if (labelBgResId != 0) {
                this.mLabelBg = getResources().getDrawable(labelBgResId);
            } else {
                int labelBgColor = mTypedArray.getColor(C0885R.styleable.labels_view_labelBackground, 0);
        ...
        if (state instanceof Bundle) {
            ...
            if (selectLabel != null && !selectLabel.isEmpty()) {
                ...
                for (int i = 0; i < size; i++) {
                    positions[i] = selectLabel.get(i).intValue();
                }
    ...
    public void setLabels(List<String> labels) {
        setLabels(labels, new LabelTextProvider<String>() { // from class: com.fimi.app.x8s.widget.LabelsView.1
            @Override // com.fimi.app.x8s.widget.LabelsView.LabelTextProvider
    ...
    private void ensureLabelClickable() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            label.setClickable((this.mLabelClickListener == null && this.mSelectType == SelectType.NONE) ? false : true);
        }
    }
    ...
    public <T> List<T> getSelectLabelDatas() {
        ...
        for (int i = 0; i < size; i++) {
            View label = getChildAt(this.mSelectLabels.get(i).intValue());
