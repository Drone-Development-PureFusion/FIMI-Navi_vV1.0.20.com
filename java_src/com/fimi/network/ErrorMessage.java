...
public class ErrorMessage {
    ...
    public static String getSystemErrorMessage(Context context, String code) {
        if ("90000".equals(code)) {
            return context.getResources().getString(C1583R.string.error_system_system_normal_response);
        }
        if (SYSTEM_ERROR_DATABASE_EXCEPTION.equals(code)) {
            return context.getResources().getString(C1583R.string.error_system_client_data_illegal_data);
        }
        if (CLIENT_DATA_ILLEGAL_DATA.equals(code)) {
            return context.getResources().getString(C1583R.string.error_system_client_data_illegal_data);
        }
        if (MD5_SIGNATURE_VERIFICATION_CANNOT_PASS.equals(code)) {
            return context.getResources().getString(C1583R.string.error_system_md5_signature_verification_cannot_pass);
        }
        return null;
    }
    ...
    public static String getUserModeErrorMessage(Context context, String code) {
        ...
        if (systrmError == null) {
            if (MOBILE_PHONES_CANNOT_BE_EMPTY.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_mobile_phones_cannot_be_empty);
            }
            if (PHONE_NUMBER_NOT_RIGHT_FORMAT.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_phone_number_not_right_format);
            }
            if (EMAIL_NOT_RIGHT_FORMAT.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_email_not_right_format);
            }
            if (USER_EXIST.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_user_exist);
            }
            if (VERIFICATION_CODE_ERROR.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_verification_code_error);
            }
            if (THE_VERIFICATION_CODE_EXPIRED.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_the_verification_code_expired);
            }
            if (THE_VERIFICATION_CODE_MADE.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_the_verification_code_made);
            }
            if (THE_USER_PASSWORD_INCORRECT.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_the_user_password_incorrect);
            }
            if (EMAIL_NOT_EMPTY.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_email_not_empty);
            }
            if (PASSWORD_NOT_EMPTY.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_password_not_empty);
            }
            if (PASSWORD_INCONSISTENT.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_password_inconsistent);
            }
            if (EMAIL_INPUT_TOO_LONG.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_email_input_too_long);
            }
            if (USER_ALREADY_EXIST.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_user_already_exist);
            }
            if (VERIFICATION_CODE_CANNOT_EMPTY.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_verification_code_cannot_empty);
            }
            if (VERIFICATION_CODE_GET_FREQUENTLY.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_verification_code_get_frequently);
            }
            if (VERIFICATION_CODE_LOGIN_OUTTIME.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_verification_code_login_outtime);
            }
            if (NO_WAY_TO_KNOW_HOW_TO_LOG_IN.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_no_way_to_know_how_to_log_in);
            }
            if (MOBILE_PHONE_FORMAT_IS_INCORRECT.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_mobile_phone_format_is_incorrect);
            }
            if (NOT_ENOUGH_PASSWORD_LENGTH.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_not_enough_password_length);
            }
            if (TWO_CIPHER_INCONSISTENCIES.equals(code)) {
                return context.getResources().getString(C1583R.string.error_user_two_cipher_inconsistencies);
            }
            if (TWO_USER_RIGHT_APPLY.equals(code)) {
                return context.getResources().getString(C1583R.string.libperson_apply_repetition);
            }
            return null;
        }
        return systrmError;
    ...
    public static String getFimwareUpdateErrorMessage(Context context, String code) {
        ...
        if (systrmError == null) {
            if (AIRCRAFT_ID_CANNOT_BE_EMPTY.equals(code)) {
                return context.getResources().getString(C1583R.string.error_fimware_update_aircraft_id_cannot_empty);
            }
            return null;
        }
        return systrmError;
