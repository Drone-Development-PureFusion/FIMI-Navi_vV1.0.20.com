...
public class WiFiUtils {
    ...
    public static boolean netOkay(Context context) {
        ...
        if (mMobileNetworkInfo != null) {
            boolean isAvailable = (mMobileNetworkInfo.getState() == NetworkInfo.State.CONNECTED) & mMobileNetworkInfo.isAvailable();
            boolean isAvailable2 = wifiWorkInfo.isAvailable();
            if (wifiWorkInfo.getState() != NetworkInfo.State.CONNECTED) {
                z = false;
            }
            return isAvailable | (z & isAvailable2);
        }
        return false;
    }
