    ...
    public LinkPacket unPacket(int c) {
        ...
        switch (this.state) {
            ...
            case STX:
                if (msg_received) {
                    ...
                    if (c != 3) {
                        ...
                        this.version = c;
                        this.f182m.getHeader().setVersion((byte) this.version);
            ...
            case LENT_TYPE_CFG1:
                ...
                this.f182m.getHeader().setLenTypeCfg(this.lenTypeCfg);
                this.state = State.LENT_TYPE_CFG2;
                this.index = this.f182m.getHeader().getDataLen();
                break;
            case LENT_TYPE_CFG2:
                this.srcId = c;
                this.f182m.getHeader().setSrcId((byte) this.srcId);
                this.state = State.SRC_ID;
                break;
            case SRC_ID:
                this.subSrcId = c;
                this.f182m.getHeader().setSubSrcId((byte) this.subSrcId);
                this.state = State.SUB_SRC_ID;
                break;
            case SUB_SRC_ID:
                this.destId = c;
                this.f182m.getHeader().setDestId((byte) this.destId);
                this.state = State.DEST_ID;
                break;
            case DEST_ID:
                this.subDestId = c;
                this.f182m.getHeader().setSubDestId((byte) this.subDestId);
                this.state = State.SUB_DEST_ID;
                break;
            case SUB_DEST_ID:
                this.encryptType = c;
                this.f182m.getHeader().setEncryptType((byte) this.encryptType);
            ...
            case RESERVE2:
                ...
                this.f182m.getHeader().setReserver(this.reserver);
            ...
            case SEQ1:
                ...
                this.f182m.getHeader().setSeq(this.seq);
            ...
            case CRC_HEADER1:
                ...
                this.f182m.getHeader().setCrcHeader(this.crcHeader);
            ...
            case CRC_FRAME3:
                ...
                this.f182m.getHeader().setCrcFrame(this.crcFrame);
                this.f182m.getHeader().unPacket();
                this.state = State.CRC_FRAME4;
                break;
            case CRC_FRAME4:
                try {
                    if (this.index <= 0) {
                        ...
                        this.f182m.payload.add((byte) c);
                        this.index--;
                        if (this.index == 0) {
                            if (this.f182m.payload.size() == this.f182m.getHeader().getPayloadLen()) {
                                this.state = State.PAYLOAD;
                                int payloadCRC = this.f182m.payload.getIntCrc();
                                if (this.f182m.getHeader().checkCRC(payloadCRC)) {
                                    this.f182m.setMsgId(this.f182m.payload.getMsgId());
                                    this.f182m.setMsgGroupId(this.f182m.payload.getGroupId());
                                    msg_received = true;
                                }
                            }
                            this.state = State.IDLE;
                    ...
                    String str = " h " + this.f182m.getHeader().getPayloadLen() + " p " + this.f182m.payload.size();
                    ByteHexHelper.bytesToHexString(this.f182m.encodePacket());
                    String x = null;
                    x.toString();
                    break;
                }
