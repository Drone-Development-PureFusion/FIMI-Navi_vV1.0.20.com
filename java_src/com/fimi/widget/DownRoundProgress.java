...
public class DownRoundProgress extends View {
    ...
    public DownRoundProgress(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.textColor = getResources().getColor(C1787R.C1789color.dialog_item_color);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.digitalEnbale) {
            ...
            float startY = (oval.height() / 2.0f) + (this.mBound.height() / 2);
            float startX = (oval.width() / 2.0f) - (this.mBound.width() / 2);
            canvas.drawText(text, startX, startY, this.paint);
        }
    }
    ...
    public void setProgressTextSize(int size) {
        this.textSize = dip2px(getContext(), size);
    }
}
