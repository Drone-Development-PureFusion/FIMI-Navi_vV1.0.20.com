    ...
    public static final class Builder<T> {
        ...
        private void parseMethodAnnotation(Annotation annotation) {
            if (annotation instanceof DELETE) {
                ...
                parseHttpMethodAndPath(HttpProxyConstants.GET, ((GET) annotation).value(), false);
                ...
                parseHttpMethodAndPath(HttpProxyConstants.PUT, ((PUT) annotation).value(), true);
        ...
        private ParameterHandler<?> parseParameterAnnotation(int p, Type type, Annotation[] annotations, Annotation annotation) {
            if (annotation instanceof Url) {
                ...
                if (Iterable.class.isAssignableFrom(rawParameterType3)) {
                    ...
                    return new ParameterHandler.Header(name3, converter5).iterable();
                } else if (rawParameterType3.isArray()) {
                    Converter<?, String> converter6 = this.retrofit.stringConverter(ServiceMethod.boxIfPrimitive(rawParameterType3.getComponentType()), annotations);
                    return new ParameterHandler.Header(name3, converter6).array();
                } else {
                    Converter<?, String> converter7 = this.retrofit.stringConverter(type, annotations);
                    return new ParameterHandler.Header(name3, converter7);
                }
                ...
                if (Iterable.class.isAssignableFrom(rawParameterType5)) {
                    ...
                    return new ParameterHandler.Field(name4, converter8, encoded2).iterable();
                } else if (rawParameterType5.isArray()) {
                    Converter<?, String> converter9 = this.retrofit.stringConverter(ServiceMethod.boxIfPrimitive(rawParameterType5.getComponentType()), annotations);
                    return new ParameterHandler.Field(name4, converter9, encoded2).array();
                } else {
                    Converter<?, String> converter10 = this.retrofit.stringConverter(type, annotations);
                    return new ParameterHandler.Field(name4, converter10, encoded2);
                }
                ...
                Headers headers = Headers.m34of("Content-Disposition", "form-data; name=\"" + partName + "\"", "Content-Transfer-Encoding", part.encoding());
                if (Iterable.class.isAssignableFrom(rawParameterType7)) {
                    if (!(type instanceof ParameterizedType)) {
                        throw parameterError(p, rawParameterType7.getSimpleName() + " must include generic type (e.g., " + rawParameterType7.getSimpleName() + "<String>)", new Object[0]);
                    }
                    ...
                    Converter<?, RequestBody> converter11 = this.retrofit.requestBodyConverter(iterableType, annotations, this.methodAnnotations);
                    return new ParameterHandler.Part(headers, converter11).iterable();
                } else if (rawParameterType7.isArray()) {
                    Class<?> arrayComponentType = ServiceMethod.boxIfPrimitive(rawParameterType7.getComponentType());
                    if (MultipartBody.Part.class.isAssignableFrom(arrayComponentType)) {
                        throw parameterError(p, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.", new Object[0]);
                    }
                    Converter<?, RequestBody> converter12 = this.retrofit.requestBodyConverter(arrayComponentType, annotations, this.methodAnnotations);
                    return new ParameterHandler.Part(headers, converter12).array();
                } else if (MultipartBody.Part.class.isAssignableFrom(rawParameterType7)) {
                    throw parameterError(p, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.", new Object[0]);
                } else {
                    Converter<?, RequestBody> converter13 = this.retrofit.requestBodyConverter(type, annotations, this.methodAnnotations);
                    return new ParameterHandler.Part(headers, converter13);
                }
                ...
                try {
                    Converter<?, RequestBody> converter14 = this.retrofit.requestBodyConverter(type, annotations, this.methodAnnotations);
                    this.gotBody = true;
                    return new ParameterHandler.Body(converter14);
                } catch (RuntimeException e) {
