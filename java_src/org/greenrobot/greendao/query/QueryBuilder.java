...
public class QueryBuilder<T> {
    ...
    public QueryBuilder<T> stringOrderCollation(String stringOrderCollation) {
        if (this.dao.getDatabase().getRawDatabase() instanceof SQLiteDatabase) {
            if (stringOrderCollation != null && !stringOrderCollation.startsWith(" ")) {
    ...
    public QueryBuilder<T> whereOr(WhereCondition cond1, WhereCondition cond2, WhereCondition... condMore) {
        this.whereCollector.add(m7or(cond1, cond2, condMore), new WhereCondition[0]);
        return this;
    }

    /* renamed from: or */
    public WhereCondition m7or(WhereCondition cond1, WhereCondition cond2, WhereCondition... condMore) {
    ...
    protected StringBuilder append(StringBuilder builder, Property property) {
        ...
        builder.append(this.tablePrefix).append(CoreConstants.DOT).append(CoreConstants.SINGLE_QUOTE_CHAR).append(property.columnName).append(CoreConstants.SINGLE_QUOTE_CHAR);
        return builder;
    }
    ...
    private void checkLog(String sql) {
        if (LOG_SQL) {
            DaoLog.m27d("Built SQL for query: " + sql);
        }
        if (LOG_VALUES) {
            DaoLog.m27d("Values for query: " + this.values);
        }
    }
