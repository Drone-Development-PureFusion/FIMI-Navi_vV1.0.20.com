...
public class AsyncOperationExecutor implements Runnable, Handler.Callback {
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                DaoLog.m18w(Thread.currentThread().getName() + " was interruppted", e);
                return;
            } finally {
                this.executorRunning = false;
            }
    ...
    private void mergeTxAndExecute(AsyncOperation operation1, AsyncOperation operation2) {
        ...
        for (int i = 0; i < mergedOps.size(); i++) {
            try {
                ...
                try {
                    ...
                    DaoLog.m22i("Async transaction could not be ended, success so far was: false", e);
                }
            }
        }
        if (success) {
        ...
        DaoLog.m23i("Reverted merged transaction because one of the operations failed. Executing operations one by one instead...");
