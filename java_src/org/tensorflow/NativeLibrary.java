...
public final class NativeLibrary {
    ...
    private static String maybeAdjustForMacOS(String libFilename) {
        if (System.getProperty("os.name").contains("OS X")) {
            ...
            return (cl.getResource(makeResourceName(libFilename)) != null || !libFilename.endsWith(".dylib")) ? libFilename : libFilename.substring(0, libFilename.length() - ".dylib".length()) + ".so";
        }
        return libFilename;
    }
    ...
    private static String extractResource(InputStream resource, String resourceName, String extractToDirectory) throws IOException {
        ...
        long nbytes = copy(resource, dst);
        log(String.format("copied %d bytes to %s", Long.valueOf(nbytes), dstPath));
        return dstPath;
    }

    /* renamed from: os */
    private static String m3os() {
    ...
    private static String makeResourceName(String baseName) {
        return "org/tensorflow/native/" + String.format("%s-%s/", m3os(), architecture()) + baseName;
    }
