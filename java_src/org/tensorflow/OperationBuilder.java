...
public final class OperationBuilder {
    ...
    public OperationBuilder(Graph graph, String type, String name) {
        this.graph = graph;
    ...
    public OperationBuilder addInput(Output<?> input) {
        Graph.Reference r = this.graph.ref();
        try {
            addInput(this.unsafeNativeHandle, input.m2op().getUnsafeNativeHandle(), input.index());
            return this;
        } finally {
            r.close();
        }
    ...
    public OperationBuilder addInputList(Output<?>[] inputs) {
        ...
        try {
            ...
            for (int i = 0; i < inputs.length; i++) {
                opHandles[i] = inputs[i].m2op().getUnsafeNativeHandle();
    ...
    public OperationBuilder setAttr(String name, String value) {
        setAttr(name, value.getBytes(Charset.forName(SocketOption.DEFAULT_CHARSET)));
        return this;
    }
    ...
    public OperationBuilder setAttr(String name, DataType value) {
        Graph.Reference r = this.graph.ref();
        try {
            setAttrType(this.unsafeNativeHandle, name, value.m4c());
            return this;
        } finally {
            r.close();
        }
    ...
    public OperationBuilder setAttr(String name, String[] value) {
        Charset utf8 = Charset.forName(SocketOption.DEFAULT_CHARSET);
