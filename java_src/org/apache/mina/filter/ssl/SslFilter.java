...
public class SslFilter extends IoFilterAdapter {
    ...
    public static final AttributeKey SSL_SESSION = new AttributeKey(SslFilter.class, C2216b.f436ac);
    ...
    public void exceptionCaught(IoFilter.NextFilter nextFilter, IoSession session, Throwable cause) throws Exception {
        if (cause instanceof WriteToClosedSessionException) {
            ...
            while (true) {
                if (!it.hasNext()) {
                    break;
                } else if (isCloseNotify(it.next().getMessage())) {
    ...
    public void filterClose(final IoFilter.NextFilter nextFilter, final IoSession session) throws SSLException {
        SslHandler future = (SslHandler) session.getAttribute(SSL_HANDLER);
        if (future != null) {
            future = null;
            try {
                try {
                    synchronized (future) {
                        if (isSslStarted(session)) {
                            future = initiateClosure(nextFilter, session);
                            future.mo1923addListener((IoFutureListener<?>) new IoFutureListener<IoFuture>() { // from class: org.apache.mina.filter.ssl.SslFilter.1
    ...
    private WriteFuture initiateClosure(IoFilter.NextFilter nextFilter, IoSession session) throws SSLException {
        ...
        try {
            ...
            WriteFuture future2 = future == null ? DefaultWriteFuture.newWrittenFuture(session) : future;
            try {
                if (sslHandler.isInboundDone()) {
                    sslHandler.destroy();
                }
