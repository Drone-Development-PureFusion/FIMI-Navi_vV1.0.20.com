...
public class DemuxingProtocolEncoder implements ProtocolEncoder {
    ...
    private MessageEncoder<Object> findEncoder(State state, Class<?> type, Set<Class<?>> triedClasses) {
        ...
        MessageEncoder<Object> messageEncoder = (MessageEncoder) state.type2encoder.get(type);
        if (messageEncoder == null) {
            ...
            for (Class<?> element : interfaces) {
                messageEncoder = findEncoder(state, element, triedClasses);
                if (messageEncoder != null) {
                    break;
                }
            }
        }
        if (messageEncoder == null && (superclass = type.getSuperclass()) != null) {
            messageEncoder = findEncoder(state, superclass);
        }
        if (messageEncoder != null) {
            state.findEncoderCache.put(type, messageEncoder);
            MessageEncoder<Object> tmpEncoder = (MessageEncoder) state.findEncoderCache.putIfAbsent(type, messageEncoder);
            if (tmpEncoder != null) {
                messageEncoder = tmpEncoder;
            }
        }
