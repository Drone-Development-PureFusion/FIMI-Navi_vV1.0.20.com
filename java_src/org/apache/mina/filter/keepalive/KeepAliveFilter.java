...
public class KeepAliveFilter extends IoFilterAdapter {
    ...
    private void markStatus(IoSession session) {
        session.mo1951getConfig().setIdleTime(this.interestedIdleStatus, 0);
        session.mo1951getConfig().setReaderIdleTime(getRequestTimeout());
        session.setAttribute(this.WAITING_FOR_RESPONSE);
    }

    private void resetStatus(IoSession session) {
        session.mo1951getConfig().setReaderIdleTime(0);
        session.mo1951getConfig().setWriterIdleTime(0);
        session.mo1951getConfig().setIdleTime(this.interestedIdleStatus, getRequestInterval());
