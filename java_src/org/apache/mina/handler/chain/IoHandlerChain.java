...
public class IoHandlerChain implements IoHandlerCommand {
    ...
    /* renamed from: id */
    private final int f1475id;
    private final Map<String, Entry> name2entry = new ConcurrentHashMap();
    private final Entry head = new Entry(null, null, "head", createHeadCommand());
    private final Entry tail = new Entry(this.head, null, "tail", createTailCommand());

    public IoHandlerChain() {
        int i = nextId;
        nextId = i + 1;
        this.f1475id = i;
        this.NEXT_COMMAND = IoHandlerChain.class.getName() + CoreConstants.DOT + this.f1475id + ".nextCommand";
