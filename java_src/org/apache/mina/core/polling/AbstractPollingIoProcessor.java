...
public abstract class AbstractPollingIoProcessor<S extends AbstractIoSession> implements IoProcessor<S> {
    ...
    private String nextThreadName() {
        ...
        return cls.getSimpleName() + CoreConstants.DASH_CHAR + newThreadId;
    }
    ...
    public final void dispose() {
        if (!this.disposed && !this.disposing) {
            ...
            this.disposalFuture.mo1925awaitUninterruptibly();
            this.disposed = true;
        }
    }
    ...
    private void read(S session) {
        int ret;
        IoSessionConfig config = session.mo1951getConfig();
    ...
    private boolean flushNow(S session, long currentTime) {
        ...
        boolean hasFragmentation = session.getTransportMetadata().hasFragmentation();
        WriteRequestQueue writeRequestQueue = session.getWriteRequestQueue();
        int maxWrittenBytes = session.mo1951getConfig().getMaxReadBufferSize() + (session.mo1951getConfig().getMaxReadBufferSize() >>> 1);
        int writtenBytes = 0;
