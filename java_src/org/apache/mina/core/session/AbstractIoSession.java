...
public abstract class AbstractIoSession implements IoSession {
    ...
    private long sessionId = idGenerator.incrementAndGet();
    ...
    public AbstractIoSession(IoService service) {
        ...
        this.closeFuture.mo1923addListener((IoFutureListener<?>) SCHEDULED_COUNTER_RESETTER);
    }
    ...
    protected void destroy() {
        ...
        if (this.writeRequestQueue != null) {
            while (!this.writeRequestQueue.isEmpty(this)) {
                ...
                if (writeRequest != null && (writeFuture = writeRequest.getFuture()) != null) {
    ...
    public final ReadFuture read() {
        ReadFuture future;
        if (!mo1951getConfig().isUseReadOperation()) {
    ...
    public WriteFuture write(Object message, SocketAddress remoteAddress) {
        ...
        try {
            ...
            if (openedFileChannel != null) {
                ...
                writeFuture.mo1923addListener(new IoFutureListener<WriteFuture>() { // from class: org.apache.mina.core.session.AbstractIoSession.2
                    @Override // org.apache.mina.core.future.IoFutureListener
    ...
    public final void updateThroughput(long currentTime, boolean force) {
        ...
        long minInterval = mo1951getConfig().getThroughputCalculationIntervalInMillis();
    ...
    public final void increaseReadBufferSize() {
        int newReadBufferSize = mo1951getConfig().getReadBufferSize() << 1;
        if (newReadBufferSize <= mo1951getConfig().getMaxReadBufferSize()) {
            mo1951getConfig().setReadBufferSize(newReadBufferSize);
        } else {
            mo1951getConfig().setReadBufferSize(mo1951getConfig().getMaxReadBufferSize());
        }
    ...
    public final void decreaseReadBufferSize() {
        ...
        if (mo1951getConfig().getReadBufferSize() > mo1951getConfig().getMinReadBufferSize()) {
            mo1951getConfig().setReadBufferSize(mo1951getConfig().getReadBufferSize() >>> 1);
        }
        this.deferDecreaseReadBuffer = true;
    }
    ...
    public final int getIdleCount(IdleStatus status) {
        if (mo1951getConfig().getIdleTime(status) == 0) {
            if (status == IdleStatus.BOTH_IDLE) {
                this.idleCountForBoth.set(0);
            }
    ...
    public SocketAddress mo1954getServiceAddress() {
        IoService service = getService();
        return service instanceof IoAcceptor ? ((IoAcceptor) service).mo1948getLocalAddress() : getRemoteAddress();
    }
    ...
    public String toString() {
        ...
        if (isConnected() || isClosing()) {
            ...
            if (getService() instanceof IoAcceptor) {
                return "(" + getIdAsString() + ": " + getServiceName() + ", server, " + remote + " => " + local + CoreConstants.RIGHT_PARENTHESIS_CHAR;
            }
            return "(" + getIdAsString() + ": " + getServiceName() + ", client, " + local + " => " + remote + CoreConstants.RIGHT_PARENTHESIS_CHAR;
        }
    ...
    public static void notifyIdleSession(IoSession session, long currentTime) {
        notifyIdleSession0(session, currentTime, session.mo1951getConfig().getIdleTimeInMillis(IdleStatus.BOTH_IDLE), IdleStatus.BOTH_IDLE, Math.max(session.getLastIoTime(), session.getLastIdleTime(IdleStatus.BOTH_IDLE)));
        notifyIdleSession0(session, currentTime, session.mo1951getConfig().getIdleTimeInMillis(IdleStatus.READER_IDLE), IdleStatus.READER_IDLE, Math.max(session.getLastReadTime(), session.getLastIdleTime(IdleStatus.READER_IDLE)));
        notifyIdleSession0(session, currentTime, session.mo1951getConfig().getIdleTimeInMillis(IdleStatus.WRITER_IDLE), IdleStatus.WRITER_IDLE, Math.max(session.getLastWriteTime(), session.getLastIdleTime(IdleStatus.WRITER_IDLE)));
    ...
    private static void notifyWriteTimeout(IoSession session, long currentTime) {
        ...
        long writeTimeout = session.mo1951getConfig().getWriteTimeoutInMillis();
