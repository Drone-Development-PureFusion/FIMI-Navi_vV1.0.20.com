...
public class DefaultIoFuture implements IoFuture {
    ...
    @Override // org.apache.mina.core.future.IoFuture
    /* renamed from: await */
    public IoFuture mo1924await() throws InterruptedException {
    ...
    public IoFuture mo1925awaitUninterruptibly() {
        try {
    ...
    private boolean await0(long timeoutMillis, boolean interruptable) throws InterruptedException {
        ...
        synchronized (this.lock) {
            if (this.ready || timeoutMillis <= 0) {
                ...
                while (true) {
                    try {
                        long timeOut = Math.min(timeoutMillis, (long) DEAD_LOCK_CHECK_INTERVAL);
    ...
    public IoFuture mo1923addListener(IoFutureListener<?> listener) {
        if (listener == null) {
    ...
    public IoFuture mo1926removeListener(IoFutureListener<?> listener) {
        if (listener == null) {
            throw new IllegalArgumentException("listener");
        }
        synchronized (this.lock) {
