...
public class DigestUtilities {
    ...
    static {
        try {
            ...
            SUPPORTED_QOPS = new String[]{AuthorBox.TYPE, "auth-int"};
    ...
    public static String computeResponseValue(IoSession session, HashMap<String, String> map, String method, String pwd, String charsetName, String body) throws AuthenticationException, UnsupportedEncodingException {
        ...
        if (!isMD5Sess || session.getAttribute(SESSION_HA1) == null) {
            ...
            sb.append(StringUtilities.stringTo8859_1(StringUtilities.getDirectiveValue(map, "username", true))).append(CoreConstants.COLON_CHAR);
            String realm = StringUtilities.stringTo8859_1(StringUtilities.getDirectiveValue(map, "realm", false));
            if (realm != null) {
                sb.append(realm);
            }
            sb.append(CoreConstants.COLON_CHAR).append(pwd);
            if (isMD5Sess) {
                synchronized (md5) {
                    md5.reset();
                ...
                sb2.append(CoreConstants.COLON_CHAR).append(StringUtilities.stringTo8859_1(StringUtilities.getDirectiveValue(map, "nonce", true)));
                sb2.append(CoreConstants.COLON_CHAR).append(StringUtilities.stringTo8859_1(StringUtilities.getDirectiveValue(map, "cnonce", true)));
                synchronized (md5) {
                    md5.reset();
        ...
        if ("auth-int".equalsIgnoreCase(qop)) {
            ...
            sb3.append(CoreConstants.COLON_CHAR).append(hEntity);
        }
        synchronized (md5) {
            md5.reset();
        ...
        sb4.append(CoreConstants.COLON_CHAR).append(StringUtilities.getDirectiveValue(map, "nonce", true));
        sb4.append(":00000001:");
        sb4.append(StringUtilities.getDirectiveValue(map, "cnonce", true));
        sb4.append(CoreConstants.COLON_CHAR).append(qop).append(CoreConstants.COLON_CHAR);
