    ...
    private void autoSelectAuthHandler(HttpProxyResponse response) throws ProxyAuthException {
        ...
        if (values == null || values.size() == 0) {
            ...
            while (true) {
                ...
                String proxyAuthHeader = it.next().toLowerCase();
            ...
            Iterator<HttpAuthenticationMethods> it2 = proxyIoSession.getPreferedOrder().iterator();
            while (true) {
                if (!it2.hasNext()) {
                    break;
                }
                HttpAuthenticationMethods method2 = it2.next();
                if (this.authHandler != null) {
                    break;
