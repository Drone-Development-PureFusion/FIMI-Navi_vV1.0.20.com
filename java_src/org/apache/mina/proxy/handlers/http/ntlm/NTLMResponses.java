...
public class NTLMResponses {
    public static final byte[] LM_HASH_MAGIC_CONSTANT = {75, FcCollection.MSG_GET_SURROUND_SPEED, FcCollection.MSG_SET_FOLLOW_EXIT, 33, FcCollection.MSG_SET_SURROUND_EXCUTE, 35, 36, 37};
    ...
    public static byte[] hmacMD5(byte[] data, byte[] key) throws Exception {
        ...
        for (int i = 0; i < 64; i++) {
            if (i < key.length) {
                ipad[i] = (byte) (key[i] ^ FcCollection.MSG_LOCK_MOTOR_STATE);
                opad[i] = (byte) (key[i] ^ SocksProxyConstants.V4_REPLY_REQUEST_FAILED_NO_IDENTD);
            } else {
                ipad[i] = FcCollection.MSG_LOCK_MOTOR_STATE;
