...
public final class MultipartBody extends RequestBody {
    ...
    private static final byte[] COLONSPACE = {FcCollection.MSG_SET_AUTO_PHOTO_RESUME, 32};
    ...
    private long writeOrCountBytes(@Nullable BufferedSink sink, boolean countBytes) throws IOException {
        ...
        for (int p = 0; p < partCount; p++) {
            ...
            sink.mo1889write(DASHDASH);
            sink.mo1888write(this.boundary);
            sink.mo1889write(CRLF);
            if (headers != null) {
                int headerCount = headers.size();
                for (int h = 0; h < headerCount; h++) {
                    sink.mo1902writeUtf8(headers.name(h)).mo1889write(COLONSPACE).mo1902writeUtf8(headers.value(h)).mo1889write(CRLF);
                }
            }
            MediaType contentType = body.contentType();
            if (contentType != null) {
                sink.mo1902writeUtf8("Content-Type: ").mo1902writeUtf8(contentType.toString()).mo1889write(CRLF);
            }
            long contentLength = body.contentLength();
            if (contentLength != -1) {
                sink.mo1902writeUtf8("Content-Length: ").mo1892writeDecimalLong(contentLength).mo1889write(CRLF);
            } else if (countBytes) {
    ...
    static StringBuilder appendQuotedString(StringBuilder target, String key) {
        target.append(CoreConstants.DOUBLE_QUOTE_CHAR);
        int len = key.length();
        for (int i = 0; i < len; i++) {
    ...
    public static final class Part {
        ...
        public static Part createFormData(String name, @Nullable String filename, RequestBody body) {
            ...
            return create(Headers.m34of("Content-Disposition", disposition.toString()), body);
        }
