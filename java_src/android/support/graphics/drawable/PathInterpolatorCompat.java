...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (Math.abs(this.f4mX[0]) > 1.0E-5d || Math.abs(this.f5mY[0]) > 1.0E-5d || Math.abs(this.f4mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f5mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
    ...
    public float getInterpolation(float t) {
        ...
        float tInRange = t - this.f4mX[startIndex];
        float fraction = tInRange / xRange;
        float startY = this.f5mY[startIndex];
        float endY = this.f5mY[endIndex];
