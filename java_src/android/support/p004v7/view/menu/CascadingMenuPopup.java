...
final class CascadingMenuPopup extends MenuPopup implements MenuPresenter, View.OnKeyListener, PopupWindow.OnDismissListener {
    ...
    public CascadingMenuPopup(@NonNull Context context, @NonNull View anchor, @AttrRes int popupStyleAttr, @StyleRes int popupStyleRes, boolean overflowOnly) {
        ...
        this.mMenuMaxWidth = Math.max(res.getDisplayMetrics().widthPixels / 2, res.getDimensionPixelSize(C0345R.dimen.abc_config_prefDialogWidth));
    }

    @Override // android.support.p004v7.view.menu.MenuPopup
    ...
    private void showMenu(@NonNull MenuBuilder menu) {
        ...
        if (parentInfo == null && this.mShowTitle && menu.getHeaderTitle() != null) {
            FrameLayout titleItemView = (FrameLayout) inflater.inflate(C0345R.layout.abc_popup_menu_header_item_layout, (ViewGroup) listView, false);
    ...
    private View findParentViewForSubmenu(@NonNull CascadingMenuInfo parentInfo, @NonNull MenuBuilder submenu) {
        ...
        while (true) {
            if (i >= count) {
                break;
            } else if (owner != menuAdapter.mo1576getItem(i)) {
                i++;
            } else {
                ownerPosition = i;
                break;
