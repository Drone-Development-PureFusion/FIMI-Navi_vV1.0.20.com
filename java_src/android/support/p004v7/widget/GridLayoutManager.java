...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    int getSpaceForSpanRange(int startSpan, int spanSize) {
        return (this.mOrientation != 1 || !isLayoutRTL()) ? this.mCachedBorders[startSpan + spanSize] - this.mCachedBorders[startSpan] : this.mCachedBorders[this.mSpanCount - startSpan] - this.mCachedBorders[(this.mSpanCount - startSpan) - spanSize];
    }
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        if (flexibleInOtherDir) {
            ...
            for (int i2 = 0; i2 < count; i2++) {
                ...
                measureChild(view3, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE, true);
        ...
        for (int i3 = 0; i3 < count; i3++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view4) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    wSpec = getChildMeasureSpec(totalSpaceInOther, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE, horizontalInsets, lp.width, false);
                    hSpec = View.MeasureSpec.makeMeasureSpec(maxSize - verticalInsets, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE);
                } else {
                    wSpec = View.MeasureSpec.makeMeasureSpec(maxSize - horizontalInsets, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE);
                    hSpec = getChildMeasureSpec(totalSpaceInOther, NTLMConstants.FLAG_NEGOTIATE_KEY_EXCHANGE, verticalInsets, lp.height, false);
                }
