...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: android.support.v7.util.DiffUtil.1
        ...
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f25x - o2.f25x;
            return cmpX == 0 ? o1.f26y - o2.f26y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f25x += range.oldListStart;
                snake.f26y += range.newListStart;
                Range left = rangePool.isEmpty() ? new Range() : rangePool.remove(rangePool.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f25x;
                    left.newListEnd = snake.f26y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f25x - 1;
                    left.newListEnd = snake.f26y;
                } else {
                    left.oldListEnd = snake.f25x;
                    left.newListEnd = snake.f26y - 1;
                }
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f25x + snake.size;
                    range.newListStart = snake.f26y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f25x + snake.size + 1;
                    range.newListStart = snake.f26y + snake.size;
                } else {
                    range.oldListStart = snake.f25x + snake.size;
                    range.newListStart = snake.f26y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
        ...
        for (int d = 0; d <= dLimit; d++) {
            for (int k = -d; k <= d; k += 2) {
                ...
                if (checkInFwd && k >= (delta - d) + 1 && k <= (delta + d) - 1 && forward[kOffset + k] >= backward[kOffset + k]) {
                    ...
                    outSnake.f25x = backward[kOffset + k];
                    outSnake.f26y = outSnake.f25x - k;
            ...
            for (int k2 = -d; k2 <= d; k2 += 2) {
                ...
                if (!checkInFwd && k2 + delta >= (-d) && k2 + delta <= d && forward[kOffset + backwardK] >= backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f25x = backward[kOffset + backwardK];
                    outSnake2.f26y = outSnake2.f25x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f25x != 0 || firstSnake.f26y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f25x + snake.size;
                int endY = snake.f26y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f25x + j;
                    int newItemPos = snake.f26y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f25x + snakeSize;
                int endY = snake.f26y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f25x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f25x + i, 1, this.mCallback.getChangePayload(snake.f25x + i, snake.f26y + i));
                    }
                }
                posOld = snake.f25x;
                posNew = snake.f26y;
